...
event DebugOwnerValue(uint256 value);

event DebugSpenderValue(uint256 value);

event DebugAfterOwnerValue(uint256 value);

event DebugAfterSpenderValue(uint256 value);
...
contract ERC20Test is Test {
    ...
    function testRevert_ExpiredPermit() public {
        ...
        vm.warp(block.timestamp + 1 days + 1 seconds); // fast forward one
    ...
    function testFail_InvalidBalance() public {
        ...
        vm.prank(spender);
        token.transferFrom(owner, spender, 2e18); // attempt to transfer 2
            // tokens (owner only owns 1)
    }
}
